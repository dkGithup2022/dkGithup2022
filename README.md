...
지금은 claude code script 로 이떄의 작업을 대체. 작업링크 : [스크립트모음 & 결과 프로젝트예시](https://github.com/dkGithup2022/claude_code_multimodule_script)
ㄴ>
 spring 멀티모듈에서 반복되는 부분을 작은 명령어 단위의 클로드 스크립트로 두어서 개발 과정을 압축/ 자세한건 링크로 
 
#### 25.8 건강문제로 잠깐 휴식 

#### 25.9 : 유저 관심사 반영한 검색 개발, 고전적인 검색 매커니즘 부분 보완 ( percolate query, phrase query, tokeniznig 품질 캐어 )   
기존 채용 검색에서 "카카오 뱅크" 를 검색 시, "카카오" 혹은 "토스 뱅크" 의 리소스가 "카카오 뱅크"보다 상단에 위치하는 경우를 포착.   
...
#### 25.10 : 기존 서비스 홍보 & 외국으로 채용 큐레이션 서비스 시도.
...
#### 25.11 
외국으로 remote, 빅테크 , 외국인 지원 가능한 포지션을 모아서 제공하는 것을 시도해봤지만, 실패 
기획적인 문제가 있었음.  이떄 만든 베이스로 기존 서비스를 마이그레이션 . devrunner.dev 에서 더 깔끔하고 좋은 코드에서 수행. 


11월 2주차 현재 배포 3일, 고유 접속자 800 명, 2일 연속 접속 유저 100 명 왕감사. 
